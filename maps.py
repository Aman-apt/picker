...
class BusinessList:
    ...
    #     if not os.path.exists(self.save_at):
    #         os.makedirs(self.save_at)
    #     self.dataframe().to_csv(f"output/{filename}.xlsx", index=False)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for search_for_index, search_for in enumerate(search_list):
            ...
            # output
            #########
            business_list.save_to_excel(f"preshcool_in_mumbai{search_for}".replace(' ', '_'))
            #business_list.save_to_csv(f"preshool{search_for}".replace(' ', '_'))
